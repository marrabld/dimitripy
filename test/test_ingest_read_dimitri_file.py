...
class TestDimitriIngestDimitriFiles(unittest.TestCase):
    ...
    def test_read_dimitri_sav_file(self):
        tmp_dict = libdimitripy.ingest.DimitriFiles.read_dimitri_sav_file(self.test_file, 'PARASOL')

        # Test that we can use the dictionary to make a DimitriObject
        test_object = libdimitripy.base.DimitriObject(tmp_dict)
        assert isinstance(test_object, libdimitripy.base.DimitriObject)

        assert isinstance(tmp_dict, dict), True

    def test_read_dimitri_netcdf_file(self):
        tmp_dict = libdimitripy.ingest.DimitriFiles.read_dimitri_netcdf_file(
    ...
    def test_read_dimitri_intercomparison_file(self):
        ...
        # Test that we can use the dictionary to make a DimitriObject
        test_object = libdimitripy.base.DimitriObject(tmp_dict[0])
        test_object_2 = libdimitripy.base.DimitriObject(tmp_dict[1])

        assert isinstance(test_object, libdimitripy.base.DimitriObject)
        assert isinstance(test_object_2, libdimitripy.base.DimitriObject)
